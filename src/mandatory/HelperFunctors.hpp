...
#ifndef HELPER_FUNCTORS_HPP
#define HELPER_FUNCTORS_HPP
...
struct isSorted
{
	...
	template <typename Container, typename Predicate>
	static typename Container::const_iterator culprit(Container CREF c, Predicate CREF predicate)
	{
		return std::adjacent_find(
			c.begin(),
			c.end(),
			opposite<Container, Predicate>(predicate)
		);
	}
	...
	{
		return culprit(c, predicate) == c.end();
	}
};


#endif //HELPER_FUNCTORS_HPP