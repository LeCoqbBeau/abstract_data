...
struct ListIterator : public ft::iterator<bidirectional_iterator_tag, T> {
	...
	public:
		...
		ListIterator(
			Iterator CREF x,
			ENABLE_IF_TT(IS_SAME_V(Iterator, iterator) && !IS_SAME_V(this_type, iterator), void*) = 0)
			: mpNode(x.mpNode) {}
...
class ListBase {
	...
	protected:
		...
		base_node_type REF internalNode() FT_NOTHROW {
			return mNodeAllocator.first();
		}


		base_node_type CREF internalNode() const FT_NOTHROW {
			return mNodeAllocator.first();
		}


		allocator_type REF internalAllocator() FT_NOTHROW {
			return mNodeAllocator.second();
		}


		allocator_type CREF internalAllocator() const FT_NOTHROW {
			return mNodeAllocator.second();
		}


		node_allocator_type internalNodeAllocator() {
			return node_allocator_type(internalAllocator());
		}


		node_allocator_type internalNodeAllocator() const {
			return node_allocator_type(internalAllocator());
		}
...
class list : public ListBase<T, Allocator> {
	...
	public:
		...
		list(InputIterator first, InputIterator last, allocator_type CREF allocator = allocator_type());
	...
	public:
		...
		void	merge(this_type REF other, Compare compare);
	...
	protected:
		...
		void	DoAssign(Integer n, Integer value, ft::true_type);

		template <typename InputIterator>
		void	DoAssign(InputIterator first, InputIterator last, ft::false_type);

		void	DoAssignValues(size_type n, value_type CREF value);

		template <typename Integer>
		void	DoInsert(ListNodeBase* pNode, Integer n, Integer value, ft::true_type);

		template <typename InputIterator>
		void	DoInsert(ListNodeBase* pNode, InputIterator first, InputIterator last, ft::false_type);
...
#define LIST_RELATIONAL_OPERATOR(op)    												\
	template <class T, class Allocator> 												\
	bool operator op(ft::list<T, Allocator> CREF lhs, ft::list<T, Allocator> CREF rhs)
